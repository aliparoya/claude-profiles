...
The argument has a number under it. The harness ships about 6,400 tokens of baseline system prompt; the [reference setup](./reference-setup/index.mdx) replaces it with 4,500 tokens. With `ENABLE_TOOL_SEARCH=true`, tool schemas drop from "every MCP tool upfront" to nine, totalling another ~9,100 tokens. About 15,800 input tokens are spent before the user says a word. That floor is paid every turn, every session, every account. [Measure your setup](./measure/index.mdx) walks through the captures.
...
[The context stack](./context-stack/index.mdx) is the diagnosis: what's actually in your window before your first prompt. [Measure your setup](./measure/index.mdx) is the proof: capture the harness baseline, capture your real session, diff them. [Profile structure](./profile-structure/index.mdx) is the prescription. [Build your own](./build-your-own/index.mdx) walks through assembly. [Reference setup](./reference-setup/index.mdx) shows the working configuration this site runs against.
